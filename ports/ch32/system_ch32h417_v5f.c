/* Port-owned copy of the vendor's system file for the V5F core.
 *
 * Upstream ships this per-example under <example>/V5F/User/ rather than in
 * EVT/EXAM/SRC. See system_ch32h417_v3f.c for the reasoning; this file is the
 * much shorter half, because the V5F never configures the clock tree -- the V3F
 * stub has already done it before waking this core -- so there is no
 * SystemInit() here, only SystemAndCoreClockUpdate().
 *
