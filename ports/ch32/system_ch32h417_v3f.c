/* Port-owned copy of the vendor's system file for the V3F core.
 *
 * Upstream ships this per-example under <example>/V3F/User/ rather than in
 * EVT/EXAM/SRC, because it is configuration rather than library code: the
 * SYSCLK_* macro below selects which clock tree SystemInit() programs. That is
 * why lib/ch32h417lib does not carry it and this port does, the same way
 * ports/stm32 owns system_stm32.c alongside lib/stm32lib.
 *
 * Taken verbatim from PlatformIO's framework-wch-noneos-sdk 2.30000.0
 * (System/ch32h417/v3f/), so it stays diffable against a future vendor drop.
 * The SYSCLK_* macro is supplied by mpconfigboard.mk and boot_v3f/Makefile
 * WITH a value -- see the note there; the #defines below stay commented out
 * exactly as the vendor left them.
 */
/********************************** (C) COPYRIGHT *******************************
